...
export const GET_PROD_CONFIRMATION_LOADING =
    '[BVN PRODUCT CONFIRMATION LOADING] CONFIRM PRODUCT LOADING';
export const GET_PROD_CONFIRMATION_SUCCESS =
    '[BVN PRODUCT CONFIRMATION SUCCESS] CONFIRM PRODUCT SUCCESS';
export const GET_PROD_CONFIRMATION_FAILURE =
    '[BVN PRODUCT CONFIRMATION FAILURE] CONFIRM PRODUCT FAILURE';
...
export function createNewM36User(data) {
    let { bvn, email, firstName, lastName, m36KycLevel, phoneNumber, userUuid, walletId } = data;
    const params = {
        bvn,
        email,
        firstName,
            ...
            .then((response) => {
                if (response.status === 200) {
                    if (response.data && response.data.code === '000') {
                        axios
                            .get('authserv/api/v1/onboard/verify_customer/bvn/' + bvn)
                            .then((response) => {
                                if (response.status === 200) {
                                    if (response.data && response.data.code === '000') {
                                        dispatch({
                                        ...
                                        dispatch(
                                            showMessage({
                                                message: 'M36 User Created Successfully!',
                                            })
                                        );
                                    } else {
                                        const { data } = response;
                        ...
                        dispatch(
                            showMessage({ message: 'Request Unsuccessful - User not created' })
                        );
                    }
                }
            })
...
export function getBvnOTP(param) {
    ...
    return (dispatch) => {
        ...
        //Making API calls while loading
        const request = axios.post('authserv/api/v1/onboard/send_verification_otp_existing_user/', {
            bvn: encrytped_bvn,
        });
        request
            .then((response) => {
                if (response.status === 200) {
                    ...
                    if (response.data && response.data.code === '000') {
                        ...
                        dispatch(
                            showMessage({
                                message: data.message ? data.message : 'Invalid BVN Details',
                            })
                        );
                    }
                } else {
...
export function getBvnDetail({ bvn, otp }) {
    ...
    return (dispatch) => {
        ...
        //Making API calls while loading
        const request = axios.post('authserv/api/v1/onboard/verify_otp_existing', params, {
            headers: {
                'Content-Type': 'application/json',
            },
        });
...
export function createPendingProductMakerEntry(data, user) {
    const { form, singleProduct, branchCode, address, kycLevel, fileUploadedUrls } = data;
    let { makerData } = data;
    ...
    params.type = 'BRANCH_INVESTMENT';
    params.status = 'PENDING';
    params.intent = 'Request to purchase investment for a customer';
    ...
    params.payload = makerData ? encryptAES(JSON.stringify(makerData)) : null;
    ...
    return (dispatch) => {
        ...
        request.then((response) => {
            ...
            axios
                .put('utilityserv/api/v1/maker_entry/update', params, {
                    headers: {
                        'Content-Type': 'application/json',
                    },
                })
                .then((response) => {
                    if (response) {
                        ...
                        let comments = `<div><p>The <b>(${product.productCategory.name})</b> with refrence id <b>${makerData.ref}</b> on the M36 branch module awaits your approval.</p>
                        ...
                        //Sending Mail to checker for activation
                            ...
                            .then((response) => {
                                if (response.status === 200) {
                                    ...
                                    dispatch(
                                        showMessage({
                                            message:
                                                'Product request successfully sent to checker for Approval',
                                        })
                                    );
                                } else {
                                    dispatch(
                                        showMessage({
                                            message: 'Request mail for activation not sent',
                                        })
                                    );
                                }
                            })
