...
function FuseNavVerticalGroup({classes, item, nestedLevel, role, active})
{
    // if ( item.auth && (!item.auth.includes(role.name.toLowerCase()) || (role.name !== 'guest' && item.auth.length === 1 && item.auth.includes('guest'))) )
    // {
    //     return null;
    // }
